...
@inject('kepegawaianservice', 'App\Services\KepegawaianService')
@inject('usersRepository', 'App\Repositories\Users\UsersRepository')

@php
$configData = Helper::appClasses();
$sdmId = request()->route('id');
$smdGet  = $kepegawaianservice->get('sdm/'.$sdmId, []);
$sdmRow  = ($smdGet->status() == 200) ? json_decode($smdGet->body())->data : false;
$pegawai = ($sdmRow) ? $biodataRepository->getByID($sdmRow->id) : false;
$profil  = ($pegawai) ? $usersRepository->getByID($sdmRow->id) : false;
@endphp
    ...
    <div class="col-md-12 mb-4 mb-md-2">
      <div class="alert alert-warning bg-warning text-white alert-dismissible d-flex align-items-baseline" role="alert">
        ...
        <div class="d-flex flex-column ps-1 text-white">
          ...
          <p class="mb-0">Belum registrasi akun SIMAIK .</p>
    ...
    <div class="card mb-4">
      ...
      <div class="card-body">
        <div class="d-flex align-items-start align-items-sm-center gap-4">
          <img src="{{ ($profil && isset($profil->foto)) ? asset('storage/'.$profil->foto) : asset('assets/img/avatars/0.png') }}" alt="user-avatar" class="d-block w-px-100 h-px-100 rounded" id="uploadedAvatar" />
        ...
        <div class="info-container">
          <dl class="row mt-2">
            ...
            <dd class="col-sm-9">{{ $sdmRow->nm_sdm }}</dd>

            <dt class="col-sm-3">Kelamin</dt>
            <dd class="col-sm-9">{{ ($sdmRow->jk == 'L') ? 'Laki - Laki' : 'Perempuan' }}</dd>
          ...
          <div class="tab-pane fade show active" id="navs-tab-home" role="tabpanel">
            ...
            <div class="info-container">
              <dl class="row mt-2">
                ...
                <dd class="col-sm-9">{{ ($pegawai && $pegawai->rantingKab) ? $pegawai->rantingKab->nama : '-' }}</dd>

                <dt class="col-sm-3">Kecamatan</dt>
                <dd class="col-sm-9">{{ ($pegawai && $pegawai->rantingKec) ? $pegawai->rantingKec->nama : '-' }}</dd>

                <dt class="col-sm-3">Alamat</dt>
                <dd class="col-sm-9">{{ ($pegawai && $pegawai->ranting_jalan) ? $pegawai->ranting_jalan : '-'}}</dd>

                <dt class="col-sm-3 text-truncate">Desa/Kelurahan</dt>
                <dd class="col-sm-9">{{ ($pegawai && $pegawai->ranting_desa_kel) ? $pegawai->ranting_desa_kel : '-' }}</dd>
