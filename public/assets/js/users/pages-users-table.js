...
$(function () {
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              `<a href="javascript:;" data-id="${full.id}" class="add-mentor dropdown-item" data-bs-toggle="modal" data-bs-target="#modal-mentor-add">Tetapkan Mentor</a>` +
  ...
  $(document).on('click', '.add-mentor', function () {
    ...
    $.get(`${baseUrl}json\/users\/${user_id}`, function (response) {
      const data = response.data;
      var pegawai = (data.pegawai) ? data.pegawai : {id : ''};
      var mentor = (pegawai.mentor) ? pegawai.mentor : {id : ''};
      if(!data.pegawai) {
        Swal.fire({
          title: 'Informasi Kesalahan',
          text: 'Data pegawai dengan user ini tidak ditemukan!',
          icon: 'error',
          customClass: {
