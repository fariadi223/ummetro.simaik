...
document.addEventListener('DOMContentLoaded', function (e) {
  (function () {
    ...
    if (typeof stepsValidation !== undefined && stepsValidation !== null) {
      ...
      // Form steps
      const stepsValidationFormStep0 = stepsValidationForm.querySelector('#ssoValidation');
      ...
      const multiSteps0 = FormValidation.formValidation(stepsValidationFormStep1).on('core.form.valid', function () {
        ...
        const sso = {
          username: $('#username_sso').val(),
          password: $('#password_sso').val()
        };
        $.ajax({
          data: JSON.stringify(sso),
          url: `${baseUrl}auth/simpeg/authenticate`,
          type: 'POST',
          ...
          success: function (status) {
            ...
            var tglLhr = moment(data.tgl_lahir, 'YYYY-MM-DD', 'id').format('DD/MM/YYYY');
            //var nmPrg = data.prodi.jenjang_didik + ' ' + data.prodi.nm_lemb;
            $('#nbm').val(data.nbm);
            $('#sdm').val(data.id);
            //$('#mhs_prodi_kode').val(data.prodi.kode_prodi);
            //$('#mhs_prodi_nama').val(nmPrg);

            $('#name').val(data.nm_sdm);
            $('#tgl_lahir').val(tglLhr);
            $('#tmpt_lahir').val(data.tmpt_lahir);
            $('#jk').val(data.jk);
            $('#email').val(data.email);
            $('#telepon_seluler').val(data.no_hp);
            $('#jln').val(data.jln);
            setTimeout(function () {
              formSection.unblock();
      ...
      const multiSteps2 = FormValidation.formValidation(stepsValidationFormStep2, {
        ...
        $.ajax({
          ...
          success: function (status) {
            ...
            $('.lbl-prodi-nama').html(data.mhs_prodi_nama);
            $('.lbl-nbm').html(data.nbm);
